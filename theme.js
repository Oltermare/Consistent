...
window.theme.changeThemeMode(
    `/appearance/themes/Consistent/theme.css`,
    `/appearance/themes/Consistent/themeMod/theme-dark.css`,
);
...
function mobileStyle(){
    loadStyle("/appearance/themes/Consistent/themeMod/mobile.css", "mobileTheme")
}
...
function InsertMenuItem2(selectid,selecttype){
    let commonMenu = document.getElementById("commonMenu")
    let readonly = commonMenu.querySelector(".b3-menu__item--readonly")
    let selectview = commonMenu.querySelector('[id="suboption"]')
    if(readonly){
        if(!selectview){
        commonMenu.insertBefore(SubOption(selectid,selecttype),readonly)
        commonMenu.insertBefore(MenuSeparator(),readonly)
        // 点击收起键盘按钮，不加了，取消，按添加日期是一样的
        // if (isPhone()){
        //     commonMenu.insertBefore(RecoverMenu(),readonly)
        // }
        }
    }
}
...
function SubOption (selectid,selecttype){
    ...
    button.id="suboption"
    button.className="b3-menu__item"
    button.innerHTML='<svg class="b3-menu__icon" style=""><use xlink:href="#"></use></svg><span class="b3-menu__label" style="">其他选项</span><svg class="b3-menu__icon b3-menu__icon--arrow" style="null"><use xlink:href="#iconRight"></use></svg></button>'
    button.appendChild(SubOptionMenu(selectid,selecttype))
    return button
}

function SubOptionMenu(selectid,selecttype,className = 'b3-menu__submenu') {
    let node = document.createElement('div');
    node.className = className;
...
setTimeout(() => {
    ...
    if (isPhone()) {
        ...
        setTimeout(() => ClickMonitor(), 3000);//各种列表转x
